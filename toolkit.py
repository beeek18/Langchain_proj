...
class SearchRecipesWithFiltersInput(BaseModel):
    query: str = Field(
        description="Блюдо или обобщённое описание на русском: 'плов', 'салат', или при одних только ограничениях — 'лёгкое блюдо', 'быстрый перекус'"
    )
...
class RecipeToolkit:
    ...
    def get_tools(self):
        ...
        def search_recipes(query: str, k: int = 3) -> str:
            """Ищет рецепты в базе данных БЕЗ фильтров по времени/калориям.
            Используй ПЕРВЫМ, когда пользователь назвал конкретное блюдо или ясную категорию (борщ, десерт).
            Если в запросе ТОЛЬКО ограничения (время, калории, БЖУ) без названия блюда — не вызывай этот тул;
            сразу вызови search_recipes_with_filters с обобщённым query (например «лёгкое блюдо», «быстрый ужин»)."""
        ...
        def search_recipes_with_filters(
            ...
            Вызывай сразу, если пользователь задал время/калории/БЖУ (даже без названия блюда):
            тогда query — короткая русская формулировка направления («лёгкий ужин», «быстро приготовить», «низкокалорийное»).
            Если уже вызывал search_recipes по названию и нужно сузить по фильтрам — вызови снова с тем же или уточнённым query и фильтрами."""
